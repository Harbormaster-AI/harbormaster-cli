...
module.exports = {
	...
	options: (name_or_id) => {
		...
		return new Promise(function(resolve, reject) {
			...
			const input = requestHandler.packageInputAddToken(constants.BLUEPRINT_OPTIONS);
			...
			return requestHandler.handleRequest( input, constants.BLUEPRINT_OPTIONS_REQUEST_MSG, function(err, data) {
				if ( err ) {
	...
	list: (filter) => {
		...
		return new Promise(function(resolve, reject) {
			...
			input.scopeType	= 'ALL'; // server likes uppercase enum types
			input.filter 	= 'published=true';

			return requestHandler.handleRequest( input, constants.BLUEPRINT_PKG_LIST_REQUEST_MSG, function(err, data) {
		    	if ( err ) {
		    		reject( status.error(err,
		    						util.format(constants.COMMAND_ERROR, constants.BLUEPRINT_PKG_LIST_REQUEST_MSG) ));
		    	} else {
		    		resolve( data );
		    	}
			});

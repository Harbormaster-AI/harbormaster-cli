...
let self = module.exports = {
	...
	generateSystemHelper: (appParams, blueprintName, modelId, appOptionsPath, gitParams) => {
		return new Promise(async function(resolve, reject) {
			...
			input.blueprintId 	= blueprintName;
	...
	generateSystem: (yamlFile, gitFile, appOptionsFile, modelIdentifier) => {
		...
		return new Promise(async function(resolve, reject) {
			...
			if ( genYamlAsJson != null ) {
				...
				let blueprintName, modelName, modelIdentifierToUse;
				let appParams;
				...
				blueprintName 	= appParams.techstack.name;
	    			...
	    			console.log('Model name ' + modelName + ' determined to be an existing file.  Validating/publishing model with server...');
	   				modelHandler.register(modelName, 
	   					...
	   					.then(function(result) {
    						...
    						self.internalGitHelper(appParams, blueprintName, modelIdentifierToUse, gitFile, appOptionsFile, deleteModel);
	...
	internalGitHelper: ( appParams, blueprintName, modelIdentifier, gitFile, appOptionsFile, deleteModel ) => {
		...
		return new Promise(async function(resolve, reject) {
			...
			self.generateSystemHelper( appParams, 
			            blueprintName, 
			            modelIdentifier, 
						appOptionsFile,

...
let self = module.exports = {
	...
	saveSystem: (yamlFile) => {
 	...
 	deleteSystem: (name_or_id) => {
 		return new Promise(async function(resolve, reject) {
 			const input = requestHandler.packageInputAddToken(constants.DELETE_SYSTEM);

 			input.modelId 		= name_or_id;

 			return requestHandler.handleRequest( input, constants.DELETE_SYSTEM_REQUEST_MSG, function(err, data) {
 				if ( err ) {
	...
	listSystem: () => {
		return new Promise(function(resolve, reject) {
			const input = requestHandler.packageInputAddToken(constants.SYSTEM_LIST);
			return requestHandler.handleRequest( input, constants.SYSTEM_LIST_REQUEST_MSG, function(err, data) {
				if ( err ) {
		    		reject( status.error(err,
